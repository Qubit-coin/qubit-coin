...
bool CPubKey::RecoverCompact(const uint256 &hash, const std::vector<unsigned char>& vchSig) {
    ...
    unsigned char pub[ECDSA_SIZE];
    size_t publen = ECDSA_SIZE;
...
bool CPubKey::Derive(CPubKey& pubkeyChild, ChainCode &ccChild, unsigned int nChild, const ChainCode& cc) const {
    assert(IsValid());
    assert((nChild >> 31) == 0);
    assert(size() == ECDSA_COMPRESSED_SIZE);
    ...
    unsigned char pub[ECDSA_COMPRESSED_SIZE];
    size_t publen = ECDSA_COMPRESSED_SIZE;
...
void CExtPubKey::Encode(unsigned char code[BIP32_EXTKEY_SIZE]) const {
    ...
    memcpy(code+41, pubkey.begin(), CPubKey::ECDSA_COMPRESSED_SIZE);
}
